...
from chat import SimplePostRequest
...
def find_position(table: List[List[str]], user: str, term: str) -> (int, int):
    ...
    users = next(
        col for col in cols if "なまえ" in col
    )  # don't work if there exist the user with the name 'なまえ'
    terms = next(row for row in rows if "月AM" in row)  # same as above

    # find target indices of update
    if user not in users:
        raise SimplePostRequest("ユーザー {} が見つかりません".format(user))
    row_id = users.index(user)
    if term not in terms:
        raise SimplePostRequest("期間 {} が見つかりません".format(term))
