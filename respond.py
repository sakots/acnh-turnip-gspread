...
class RespondService:
    ...
    def handle_update_request(
        ...
        if name is None:
            ...
            return response.warning(title="スプレッドシートでの名前が bot に登録されていません",
                                    description=
                                    "スプレッドシートに名前を入力してから `@[kabu] im [スプレッドシートでの名前]`"
                                    " とリプライして登録してください。")
        ...
        if isinstance(position, table.UserNotFound):
            ...
            return response.warning(title="スプレッドシートからあなたの名前が見つかりません",
        ...
        if not isinstance(position, table.Found):
            ...
            return response.error(title="スプレッドシートのどこに書けばいいか分かりません",
                                  description="開発者は確認してください。")
        ...
        try:
            ...
            return response.error(title="スプレッドシートへの書き込みに失敗しました",
                                  description="開発者は確認してください。")
    ...
    def handle_history_request(self, author: discord.Member) -> Optional[response.Response]:
        # FIXME: dup
        sheet_index = 0
        raw_table = self.gspread_service.get_table(sheet_index)
        table_service = TurnipPriceTableViewService(raw_table)
        ...
        if history is None:
            return response.warning(title="スプレッドシートからあなたの名前が見つかりません",
                                    description="bot に登録された名前 `%s` は正しいですか？" % name)
        return response.success(title="あなたの履歴です",
                                fields=[("履歴", format_history(history)),
                                        ("予測", prediction_link(history))])
    ...
    def handle_bind_request(
        ...
        try:
            ...
            return response.error(title="スプレッドシートでの名前を覚える際にエラーが発生しました",
                                  description="開発者は確認してください。")
        logger.info("successfully bound. %s is %s, ", author, request.name)
        author_str = author.nick or str(author)
        return response.success(title="覚えました",
                                description="%s はスプレッドシートで `%s`。" % (author_str, request.name))

    def handle_who_am_i_request(self, author: discord.Member) -> Optional[response.Response]:
        name = self.bind_service.find_name(author.id)
        author_str = author.nick or str(author)
        if name is not None:
            logger.info("successfully found name. author: %s, name: %s", author, name)
            return response.success(title="覚えてます",
